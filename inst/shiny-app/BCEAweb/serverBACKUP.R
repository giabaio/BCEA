...
function(input,output,session) {
  if(!is.null(parameters)) {
    ...
    shiny::observe({
      ...
      if(input$from=="R") {
        rm(list=ls())
        ## Check to see if JAGS/OpenBUGS & R2jags/R2OpenBUGS are present
        shiny::updateSelectInput(session,'parameter',choices="")
        inFile_params <- input$par_sims_R
        if (is.null(inFile_params)) {return(NULL)}
        obj <- readRDS(inFile_params$datapath)
        if(class(obj)=="rjags") {
          req(inFile_params)
          params <- obj$BUGSoutput$sims.matrix
        }
        if(class(obj)=="bugs") {
          shiny::req(inFile_params)
          params <- obj$sims.matrix
        }
        shiny::updateSelectInput(session,'parameter',choices = colnames(params))
      }
      if(input$from=="BUGS") {
      }
    })
    ...
    obj <- shiny::reactive({
        shiny::req(input$par_sims_R)
        inFile_params <- input$par_sims_R
        object <- readRDS(inFile_params$datapath)
    })

    param <- shiny::reactive({
      if(input$from=="Spreadsheet") {
        if(is.null(input$par_sims_csv)) {return(NULL)}
        inFile_params <- input$par_sims_csv
    ...
    output$hist <- shiny::renderPlot ({
      ...
      shiny::withProgress("Creating plot", value = 1,message = "Creating plot,", detail = "Please wait...", {
        hist(param()[,colm()], breaks= seq(min(param()[,colm()]),max(param()[,colm()]),l = input$bins+1),
             main = paste("Histogram of",input$parameter),xlab = input$parameter, ylim = ylim)
    ...
    output$trace <- shiny::renderPlot ({
      if(input$from=="Spreadsheet") {
        shiny::req(param(),colm())
        plot(param()[,colm()], type = "l",main = paste("Traceplot of ",input$parameter),ylab = input$parameter)
      }
      if(input$from%in%c("R","BUGS")){
        shiny::req(param(),colm())
...
#       shiny::updateSelectInput(session,'parameter',choices=names(parameters))
#     } else {
#         # Upload the file with the data & writes the name on the slidebar
#         ## 1. From spreadsheet
#         shiny::observe({
#             rm(list=ls())
#             if (input$from=="Spreadsheet") {
...
#         # Histogram for spreadsheet
#         output$hist <- shiny::renderPlot ({
#             if (is.null(param()[,colm()])) return(invisible(NULL))
#             if(length(var(param()[,colm()]))==0) {return(invisible())}
#             if (var(param()[,colm()])==0) {return(invisible())} 
#             ylim=NULL
#             shiny::withProgress("Creating plot", value = 1, 
#                                 message = "Creating plot,", detail = "Please wait...", {
#                                     hist(param()[,colm()], breaks= seq(min(param()[,colm()]),max(param()[,colm()]), 
#                                                                        l = input$bins+1),main = paste("Histogram of",input$parameter),
...
#         })
#         
#         output$trace3 <- shiny::renderPlot ({
#             if (is.null(param()[,colm()])) return(NULL)
#             plot(param()[,colm()], type = "l",main = paste("Traceplot of ",input$parameter),ylab = input$parameter)
#         })
#         
...
#         
#         obj <- shiny::reactive({
#             if(is.null(input$par_sims_R)) {return(NULL)}
#             inFile_params <- input$par_sims_R
#             obj <- readRDS(inFile_params$datapath)      
#         })
#         
#         param1 <- shiny::reactive({
#             if(is.null(input$par_sims_R)) {return(NULL)}
#             inFile_params <- input$par_sims_R
#             parameters <- obj()$BUGSoutput$sims.matrix        
#         })
...
#         })
#         
#         colm1 <- shiny::reactive ({
#             column <- which(nm1() == input$parameter)
#         })
#         
#         # Histogram for R
#         output$hist1 <- shiny::renderPlot ({
#             if (is.null(param1()[,colm1()])) return(NULL)
#             if(length(var(param1()[,colm1()]))==0) {return(invisible())}
#             if (var(param1()[,colm1()])==0) {return(invisible())}  
#             ylim=NULL
#             shiny::withProgress("Creating plot", value = 1, 
#                                 message = "Creating plot,", detail = "Please wait...", {
#                                     hist(param1()[,colm1()], breaks= seq(min(param1()[,colm1()]),max(param1()[,colm1()]), 
#                                                                          l = input$bins+1),main = paste("Histogram of",input$parameter),
#                                          xlab = input$parameter, ylim = ylim)
#                                     points(c(quantile(param1()[,colm1()],0.025),quantile(param1()[,colm1()],0.025),
#                                              quantile(param1()[,colm1()],0.975)),c(0,0,0),lwd=5,type="l")
#                                 })
#         })
#         
#         output$trace1 <- shiny::renderPlot ({
#             if (is.null(param1()[,colm1()])) return(NULL)
#             mytraceplot(input$parameter,obj())
#         })
#         
#         output$gr1 <- shiny::renderPlot ({
#             if (is.null(param1()[,colm1()])) return(NULL)
#             plotGR(obj(),colm1())
#         })
#         
...
#         })
#         
#         output$summary21 <- shiny::renderTable({
#             if (is.null(param1()[,colm1()])) return(NULL)
#             if(is.integer(colm1()) && length(colm1()) == 0L) {return(NULL)} else {
#                 tmp <- obj()$BUGSoutput$summary[colm1(),]
#                 summary21 <- data.frame(t(tmp))[-c(4,6)]
#                 summary21[8] <- obj()$BUGSoutput$summary[colm1(),"sd"]/sqrt(obj()$BUGSoutput$summary[colm1(),"n.eff"])
#                 colnames(summary21) <- c("Mean","Standard deviation","2.5%","Median","97.5%","Rhat","n.eff","Monte Carlo SE")
#                 rownames(summary21) <- input$parameter
#                 quiet(print(format(summary21,digit=5,nsmall=3)))
#             }
#         })
#         
#         
...
#                 }
#                 obj1 <- list(BUGSoutput=mcmc2bugs(coda::mcmc.list(coda_obj),program="jags"))   
#                 shiny::updateSelectInput(session,'parameter',choices = colnames(obj1$BUGSoutput$sims.matrix))
#             } 
#             return(obj1)
#         })
#         
...
#             shiny::req(obj1())
#             parameters <- obj1()$BUGSoutput$sims.matrix                      
#         })
#         
#         nm2 <- shiny::reactive({ 
#             shiny::req(param2())
...
#             if (is.null(param2()[,colm2()])) return(NULL)
#             if(length(var(param2()[,colm2()]))==0) {return(invisible())}
#             if (var(param2()[,colm2()])==0) {return(invisible())}  
#             ylim=NULL
#             shiny::withProgress("Creating plot", value = 1, 
#                                 message = "Creating plot,", detail = "Please wait...", {
#                                     hist(param2()[,colm2()], breaks= seq(min(param2()[,colm2()]),max(param2()[,colm2()]), 
#                                                                          l = input$bins+1),main = paste("Histogram of",input$parameter),
#                                          xlab = input$parameter, ylim = ylim)
...
#         })
#         
#         output$trace2 <- shiny::renderPlot ({
#             if (is.null(param2()[,colm2()])) return(NULL)
#             mytraceplot(input$parameter,obj1())
#         })
#         
#         output$gr2 <- shiny::renderPlot ({
#             if (is.null(param2()[,colm2()])) return(NULL)
#             if(input$nchains>1) {plotGR(obj1(),colm2())}
#         })
#         
#         output$neff1 <- shiny::renderPlot({
#             if (is.null(param2()[,colm2()])) return(NULL)
#             if(input$nchains>1){plot.neff(obj1(),colm2())}
#         })
#         
#         output$acf1 <- shiny::renderPlot({
#             if (is.null(param2()[,colm2()])) return(NULL)
#             acf.plot(obj1(),colm2())
#         })
#         
#         output$summary22 <- shiny::renderTable({
#             if (is.null(param2()[,colm2()])) return(NULL)
#             if(is.integer(colm2()) && length(colm2()) == 0L) {return(NULL)} else {
#                 tmp <- obj1()$BUGSoutput$summary[colm2(),]
#                 summary22 <- data.frame(t(tmp))[-c(4,6)]
#                 rownames(summary22) <- input$parameter
#                 if(input$nchains>1){
#                     summary22[8] <- obj1()$BUGSoutput$summary[colm2(),"sd"]/sqrt(obj1()$BUGSoutput$summary[colm2(),"n.eff"])
#                     colnames(summary22) <- c("Mean","Standard deviation","2.5%","Median","97.5%","Rhat","n.eff","Monte Carlo SE")
#                 } else {
#                     colnames(summary22) <- c("Mean","Standard deviation","2.5%","Median","97.5%")
#                 }
#                 quiet(print(format(summary22,digit=5,nsmall=3)))
#             }
#         })
#     }
#     
...
#         }
#         
#         ### Uploads the economic output (e,c) from R
#         else if (input$data=="R") {
#             inputs  <- shiny::reactive({
#                 inFile2 <- input$file2
#                 if(is.null(input$file2)) {return(invisible())}
#                 ws = readRDS(inFile2$datapath)
#             })
#         }
#         
#         else if (input$data=="BUGS") {
#             inputs <- shiny::reactive({
#                 rm(list=ls())
#                 # Checks if coda is installed (and if not, asks for them)
#                 if(!isTRUE(requireNamespace("coda",quietly=TRUE))) {
...
#         }
#         
#         ## Allows to use some of the PSA data already uploaded as (e,c)
#         else if (input$data=="Parameter simulations (from Check assumptions)") {
#             inputs  <- shiny::reactive({
#                 if(input$from=="Spreadsheet") {
#                     psa.data <- param() 
#                 } 
...
#                 n.cols <- as.integer(dim(inputs())[2])
#                 odds <- seq(1,n.cols,by=2)
#                 even <- seq(2,n.cols,by=2)
#                 e <- as.matrix(inputs()[,odds])
#                 c <- as.matrix(inputs()[,even])
#                 if(input$data=="BUGS" | input$data=="Parameter simulations (from Check assumptions)") {
...
#         
#         #############################################################################################################
#         ## Analysis with risk aversion & mixed strategy --- not implemented in the UI (and not needed, really...)
#         # # a vector of values for the risk aversion parameter r
#         # ra <- shiny::reactive({
#         #     if (is.null(input$file1) & is.null(inputs())) {return(invisible())}
#         #     r <- c(input$r1, input$r2, input$r3, input$r4)
#         #     av <- CEriskav(m(), r = r, comparison = 1)
#         # })
#         # # risk aversion
#         # output$aver <- shiny::renderPlot({
#         #     if (is.null(ra()))
#         #         return(NULL)
#         #     # If the willingness to pay grid is only one number then don't print the plots
#         #     wtp <- seq(input$min,input$max,by=input$step)
...
#         #     if (is.null(input$file1) & is.null(inputs())) {return(invisible())}
#         #     q1 <- 1-input$q0
#         #     shares <- c(input$q0, q1)
#         #     mix <- mixedAn(m(),mkt.shares = shares, plot = FALSE)
#         # })
#         # 
#         # # mixed strategy
#         # output$mixed <- shiny::renderPlot({
#         #     if (is.null(input$file1) & is.null(inputs())) {return(invisible())}
#         #     # If the willingness to pay grid is only one number then don't print the plots
#         #     wtp <- seq(input$min,input$max,by=input$step)
#         #     if (length(wtp)==1) {return(invisible)}
#         #     shiny::withProgress(plot(ma()), value = 1, message = "Creating plot,", detail = "Please wait...")
#         # })
#         #############################################################################################################
#         
...
#             # }  else {
#             #   withProgress({
#             #     plot(compute_evppi()$mesh$mesh,main="")
#             #     points(compute_evppi()$mesh$pts,col="blue",pch=19,cex=.8,title("INLA Mesh plot"))
#             #     ###            print(make2plots(compute_evppi()))
#             #   },
#             #   min=0, max=1, value = 1, message = "Creating plot,", detail = "Please wait..."
#             #   )
#             # }
...
#     ############
#     
#     ##### This should make pandoc available when packaging BCEAweb
#     ##### See: http://stackoverflow.com/questions/28432607/pandoc-version-1-12-3-or-higher-is-required-and-was-not-found-r-shiny
