...
if(input$from=="Spreadsheet"){n.params <- dim(param())[2]}
if(input$from=="R"){
  n.params <- dim(param1())[2]
...
if (input$from=="R") {
cat(paste0("In addition, we also report some 'convergence' diagnostics plots. \nThe leftmost graph shows a _traceplot_ of the ",obj()$n.chains," monitored Markov Chains, while the rightmost graph shows an _autocorrelation_ plot.\n\n"))
params.names <- colnames(param1())

for (i in 1:n.params) {
if (var(param1()[,i])!=0) {
cat(paste0("## Summary statistics for parameter ",params.names[i],"\n"))
hist(param1()[,i], breaks= seq(min(param1()[,i]),max(param1()[,i]),l = input$bins+1),
#main = paste('Histogram of',params.names[i]),
main="",xlab = params.names[i], ylim = NULL)
points(c(quantile(param1()[,i],0.025),quantile(param1()[,i],0.025),quantile(param1()[,i],0.975)),c(0,0,0),lwd=5,type='l')
cat("  \n")



cat("  \n")
tmp <- obj()$BUGSoutput$summary[i,]
summary21 <- data.frame(t(tmp))[-c(4,6)]
summary21[8] <- obj()$BUGSoutput$summary[i,"sd"]/sqrt(obj()$BUGSoutput$summary[i,"n.eff"])
#colnames(summary21) <- c("Mean","Standard deviation","2.5%","Median","97.5%","Rhat","n.eff","Monte Carlo SE")
#rownames(summary21) <- params.names[i]
## see discussion here: https://github.com/yihui/knitr/issues/886
print(knitr::kable(summary21,align=c('l','l','l','l','l','l','l','l'),
...
mytraceplot(params.names[i],obj())

acf.plot(obj(),i,title="")
cat("  \n\n")
}
}
}
