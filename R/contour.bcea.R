  ...
  function(he,
    ...
    if(base.graphics){
      ...
      if (he$n_comparisons == 1) {
        density <- MASS::kde2d(he$delta.e,
                               he$delta.c,
                               n = 300,
                               h = c(sd(hex$delta.e) / scale,
                                     sd(he$delta.c) / scale))
        offset <- 1.0
        
        p.ne <- sum(he$delta.e>0 & he$delta.c>0)/he$n.sim
        p.nw <- sum(he$delta.e<=0 & he$delta.c>0)/he$n.sim
        p.sw <- sum(he$delta.e<=0 & he$delta.c<=0)/he$n.sim
        p.se <- sum(he$delta.e>0 & he$delta.c<=0)/he$n.sim
        ...
        text(offset*M.e,offset*M.c,parse(text=t1),cex=0.8,pos=2)
        t2 <- paste("Pr(Delta[e]<=0, Delta[c]>0)==",format(p.nw,digits=4,nsmall=3),sep="")
        text(offset*m.e,offset*M.c,parse(text=t2),cex=0.8,pos=4)
        t3 <- paste("Pr(Delta[e]<=0, Delta[c]<=0)==",format(p.sw,digits=4,nsmall=3),sep="")
        text(offset*m.e,offset*m.c,parse(text=t3),cex=0.8,pos=4)
        t4 <- paste("Pr(Delta[e]>0, Delta[c]<=0)==",format(p.se,digits=4,nsmall=3),sep="")
        text(offset*M.e,offset*m.c,parse(text=t4),cex=0.8,pos=2)
      }
      
      if(he$n_comparisons>1) {
        if(!exists("title",where=exArgs)){title <- 
          paste("Cost effectiveness plane contour plot \n",
                he$interventions[he$ref],
                " vs ",
        ...
        density <-
          MASS::kde2d(he$delta.e[, comparison],
                      he$delta.c[, comparison],
                      n = 300,
                      h = c(sd(he$delta.e[, comparison]) / scale,
                            sd(he$delta.c[, comparison]) / scale))
        offset <- 1.0
        
        p.ne <- sum(he$delta.e[,comparison]>0 & he$delta.c[,comparison]>0)/he$n.sim
        p.nw <- sum(he$delta.e[,comparison]<=0 & he$delta.c[,comparison]>0)/he$n.sim
        p.sw <- sum(he$delta.e[,comparison]<=0 & he$delta.c[,comparison]<=0)/he$n.sim
        p.se <- sum(he$delta.e[,comparison]>0 & he$delta.c[,comparison]<=0)/he$n.sim
        ...
        # If the user has specified the range of the graph, use those values
        if(!is.null(xlim)) {m.e <- xlim[1]
        M.e <- xlim[2]}
        if(!is.null(ylim)) {m.c <- ylim[1]
        M.c <- ylim[2]}
        
        plot(he$delta.e[,comparison],he$delta.c[,comparison],pch=20,cex=.3,col="dark grey",
             xlab=xlab,ylab=ylab,
      ...
      if(!isTRUE(requireNamespace("ggplot2",quietly=TRUE)&requireNamespace("grid",quietly=TRUE))){
        ...
        contour.bcea(he,comparison=comparison,scale=scale,nlevels=nlevels,pos=alt.legend,levels=levels,graph="base",...)
      ...
      if(he$n_comparisons==1) {
        kd <- with(he,data.frame("e"=delta.e,"c"=delta.c))
      ...
      if(he$n_comparisons>1&is.null(comparison)==TRUE) {
        # creates dataframe for plotting
        kd <- with(he,data.frame(
        ...
        colors.label <- paste0("gray",round(seq(0,100,length.out=(he$n_comparisons+1))[-(he$n_comparisons+1)]))
        comparisons.label <- paste0(he$interventions[he$ref]," vs ",he$interventions[he$comp])
        do.nothing=function(x,limits) return(x)
        # plot limits
        ...
        if(!is.null(scale)&requireNamespace("MASS",quietly=TRUE)) {
          ...
          for(i in 1:he$n.comparison) {
            temp <- with(he,MASS::kde2d(delta.e[,i],delta.c[,i],n=300,h=c(sd(delta.e[,i])/scale,sd(delta.c[,i])/scale)))
      ...
      if(he$n_comparisons>1&is.null(comparison)==FALSE) {
        ...
        return(contour.bcea(he,scale=scale,pos=alt.legend,nlevels=nlevels,graph="ggplot2",comparison=NULL))
      }
