...
tabulate_means <- function(he,
  ...
  data.frame(
    lambda.e = sapply(1:he$n_comparisons,
                      function(x) mean(as.matrix(he$delta.e)[, x])),
    lambda.c = sapply(1:he$n_comparisons,
                      function(x) mean(as.matrix(he$delta.c)[, x])),
...
ceplane_plot_plotly.bcea <- function(he) {
  ...
  for (comp in seq_len(he$n_comparisons)) {
    ceplane <- plotly::add_trace(
      ...
      x = ~delta.e,
      marker = list(
        color = ifelse(grepl(pattern = "^rgba\\(", x = plot_aes$point$colors[comp]),
                       yes = plot_aes$point$colors[comp],
                       no = plotly::toRGB(plot_aes$point$colors[comp])),
