...
ib.plot.bcea <- function(he,
  ...
  if(base.graphics) {
    ...
    plot(d$x,d$y,t="l",ylab="Density",xlab=expression(paste("IB(",bold(theta),")",sep="")),main=txt,axes=FALSE,col="white",xlim=xlim)
    box()
    axis(1)
    ypt <- .95*max(d$y)
    ...
    ib <- ggplot2::ggplot(df,ggplot2::aes(x,y)) + ggplot2::theme_bw() +
      ...
      ggplot2::geom_ribbon(data=subset(df,x>0),ggplot2::aes(ymax=y),ymin=0,fill="grey50",alpha=0.2) + ggplot2::geom_line() +
      ggplot2::annotate(geom="text",label=paste0("p(IB(theta)>0,k==",wtp,")"),parse=TRUE,x=df$x[which.max(df$y)],y=max(df$y),hjust=-0.5,vjust=1,size=3.5) + ggplot2::coord_cartesian(xlim=xlim)
