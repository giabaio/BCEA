...
#' 
#' phi = a vector with simulations from the posterior distribution of the the parameter
#'       with respect to which the EVPPI is computed (phi), obtained by the full model
#' phi.name = a string containing the name of the parameter phi, as represented in the txt file
...
#' n.out = number of simulations used to compute the average with respect to phi (default = 1000)
#' m = a BCEA object in which the full economic analysis is stored as a result of the call to bcea
#' data = a list with the data to be passed to JAGS to run the model for the vector of
#'        parameters psi (conditionally on each value of the fixed parameter phi)
#' model.file = a txt file containing the full model for psi (but considering the parameter
#'              phi as fixed. This is in general a modification of the original model file)
#' params = a collection of parameters to be monitored in the conditional model for psi (given phi)
#' inits = specifies the initial values for the MCMC procedure (default = NULL)
#' n.iter = number of iterations to be run
#' n.burnin = number of iterations to be discarded as burn-in
#' n.thin = number of iterations to be considered for thinning (default NULL)
#' n.mc = number of simulations retained to compute the MC estimations (default=1000)
#' working.dir = a string containing the working directory (default is NULL)
#' ec.vars = a string with the name of the R file that contains the commands required to
...
#' bcea.call = a string containing the call to BCEA to perform the C/E evaluation. By default
