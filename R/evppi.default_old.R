...
evppi.bcea <- function (he, parameter, input, N = NULL, plot = F, residuals = T,...) {
  ...
  if (!exists("select", where=exArgs) & N < he$n_sim) {
    exArgs$select <- sample(1:he$n_sim, size = N, replace = F)
  }
  ...
  if (length(parameter) <= 4 & !exists("method", where = exArgs)) {
    exArgs$method <- list(rep("GAM",he$n_comparators-1),rep("GAM",he$n_comparators-1))
  }
  if (length(parameter) > 4 & !exists("method", where = exArgs)) {
    exArgs$method <- list(rep("INLA",he$n_comparators-1),rep("INLA",he$n_comparators-1))
  }
  if(class(exArgs$method)!="list"){
    if(exArgs$method=="sad"|exArgs$method=="so"){
      ...
      if(length(exArgs$method)==1){
        exArgs$method <- list(rep(exArgs$method,he$n_comparators-1),rep(exArgs$method,he$n_comparators-1))
      }
    }
  }
  ...
  if(!exists("int.ord",where=exArgs)){
    exArgs$int.ord <- list(rep(1,he$n_comparators-1),rep(1,he$n_comparators-1))
  }
  if(class(exArgs$int.ord)!="list"){
    exArgs$int.ord <- list(rep(exArgs$int.ord[1],he$n_comparators-1),rep(exArgs$int.ord[2],he$n_comparators-1))
  }
  
  prep.x<-function(he,select,k,l){
    if(k==1){
      x<-as.matrix(he$delta_e)[select,l]
    }
    if(k==2){
      x<-as.matrix(he$delta_c)[select,l]
    }
    return(x)
  }
  ...
  estimate.hyperparameters <- function(x, input.matrix, parameter,n_sim) {
    p <- length(parameter)
    initial.values <- rep(0, p + 1)
    repeat {
      log.hyperparameters <- optim(initial.values,
                                   fn = post.density,parameter=parameter, x = x[1:n_sim],
                                   input.matrix = input.matrix[1:n_sim, ],
  ...
  fit.gp <- function(parameter, inputs, x, n_sim) {
    tic <- proc.time()
    p <- length(parameter)
    ...
    H <- cbind(1, input.matrix)
    q <- ncol(H)
    hyperparameters <- estimate.hyperparameters(x = x,input = input.matrix, parameter = parameter, n_sim = n_sim)
  ...
  if(class(exArgs$method)=="list"){
    ...
    fit.full<-list()
    fit.full[[1]]<-matrix(data=0,nrow=length(exArgs$select),ncol=he$n_comparators)
    fit.full[[2]]<-matrix(data=0,nrow=length(exArgs$select),ncol=he$n_comparators)
    for(k in 1:2){
      for(l in 1:he$n_comparisons){
        ...
        if (method == "gp" || method == "GP") {
          ...
          fit <- fit.gp(parameter = parameter, inputs = inputs, x = x, n_sim = n_sim)
        }
        if (method == "INLA") {
          method <- "INLA"
